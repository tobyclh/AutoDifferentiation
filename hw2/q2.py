...
for timestep in TIMESTEPS:
    total_steps = int(np.ceil((T_end - T_0) / timestep))
    ...
    for func in [euler, Ruken]:
        ...
        for i in range(0, total_steps-1):
            ...
            losses[i+1] = np.abs(real_func(np.round(ts[i+1], 5)) - ys[i+1])
            if np.isclose([0.2, 0.3, 0.4], t_n).any():
                print(
                    f'{func.__name__}\t{t_n}\t{timestep}\t{ys[i+1]}\t{losses[i+1]}')
        plt.plot(ts, losses, label=f"{func.__name__}, dt={timestep}")
